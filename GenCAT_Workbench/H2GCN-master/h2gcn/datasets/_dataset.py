...
class PlanetoidData:
    ...
    def load_data(self, dataset_str, dataset_path="/content/drive/My Drive/Colab Notebooks/GenCAT/", save_plot=None, val_size=None):
        """
        ...
        test_idx_reorder = self.parse_index_file(
            "{}/ind.{}.test.index".format(dataset_path, dataset_str))
        test_idx_range = np.sort(test_idx_reorder)

        if dataset_str == "cora" or dataset_str == "citeseer" or dataset_str == "pubmed":
            idx_val = range(len(y), len(y)+500)
        else:
            val_idx_reorder = self.parse_index_file("{}/ind.{}.val.index".format(dataset_path, dataset_str))
