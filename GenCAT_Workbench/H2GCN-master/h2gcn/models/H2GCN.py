...
def add_subparser_args(parser):
    ...
    subparser.add_argument("--l2_regularize_weight", type=float, default=5e-4,
                           help="(default: %(default)s)")
    subparser.add_argument("--early_stopping", type=int, default=200,
    ...
    subparser.add_argument("--print_per_epoch", type=bool, default=False)
    subparser.add_argument("--iter_count", type=int, default=0)
    subparser.add_argument("--root_path", type=str, default='/content/drive/My Drive/Colab Notebooks/GenCAT/')
...
def initialize_model(args, layer_setups, optimizer, lr,
    ...
    def post_train_callback(args):
        ...
        return args.objects["best_val_stats"]['test_accuracy'].numpy()
        # with open(args.root_path+"experimental_results/H2GCN_"+args.dataset[4:]+str(args.iter_count), 'w') as f:
        #   f.write(str(args.objects["best_val_stats"]['test_accuracy'].numpy()) + "\n" + str(elapsed_time) + "[sec]")
        # print(args.objects["best_val_stats"]['test_accuracy'].numpy())
